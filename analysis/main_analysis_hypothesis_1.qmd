...
job_demands = data |>
  select(anonym_id, WLD, PPA, PPS, AMB, CON, WLB)
...
job_resources = data |>
  select(anonym_id, OPP, MNG, INF, SOC, PAY, TOL, GNL, PPR, SEC)
...
```{r}
data_normality = data |> 
  select(burnout:WLB, -open_practice, -qrp_attitude) |>
...
data_no_outliers <- outlier_df %>% 
  ...
  select(-c(cooks_d, is_outlier_cooks))  # remove diagnostics columns
...
## PARAMETERS
### Factor loadings 0.3 (chosen as a conservative estimate based on lowest loading in pilot model (pay_satisfaction = 0.294))
### Regressions 0.3 and -0.3 (chosen as a conservative estimate based on lowest loading in pilot model (job_resource = -0.382))
### Variances 0.91 (calculated by pwrSEM)
## Covariances -0.5 (chosen as a conservative estimate based on lowest covariance in pilot model (job_demands ~~ job_resources = -0.690))

## Result of analysis based on 1100 simulations
### Effects of interest: burnout ~ job demands, burnout ~ job resources, job demands ~~ job resources
...
jdr_final_fit = sem(jdr_final_model, data = data_no_outliers, std.lv = TRUE, estimator = "MLR", missing = "fiml")
