...
for (col in role_stressor_columns) {
  role_stressor_data[[col]] = case_when(
...
role_ambiguity_data = role_stressor_data |> 
  select(role_ambiguity_1:role_ambiguity_6, anonym_id)

role_conflict_data = role_stressor_data |> 
...
processed_data = burnout_data |> 
  ...
  left_join(role_ambiguity_data, by = "anonym_id") |> 
  left_join(role_conflict_data, by = "anonym_id") |> 
...
#Save data
```{r}
write.csv(processed_data, here::here("data/clean_to_processed_data.csv"), row.names = FALSE)
